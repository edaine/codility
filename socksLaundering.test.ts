...
import { solution } from "./socksLaundering";

describe("solution template", () => {
  // --- Examples from the problem description ---
  test("Basic example 1", () => {
    expect(solution(2, [1, 2, 1, 1], [1, 4, 3, 2, 4])).toBe(3);
  });

  test("Basic example 2", () => {
    expect(solution(4, [1, 2, 1, 1], [1, 4, 3, 2, 4])).toBe(4);
  });

  test("Basic example 3", () => {
    expect(solution(50, new Array(50).fill(1), new Array(50).fill(1))).toBe(50);
  });
});
